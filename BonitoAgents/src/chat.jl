...
mutable struct ChatModel
    ...
    # Timestamp of the last inbound ACP frame (stream activity). The busy
    # heuristic (`update_busy!`) uses this to detect a "quiet" period after
    # the agent stops streaming — the spinner clears 3 s after the last
    # chunk, not when the turn loop exits (which may lag if the turn has
    # post-turn bookkeeping).
    last_stream_at::Ref{Float64}
...
function chat_header(session::Bonito.Session, model::ChatModel, sync_modal_state::Observable)
    ...
    on(session, provider_select) do val
        isempty(val) && return
...
function handle_command!(model::ChatModel, session::Session, cmd::ToolRenderCommand)
    ...
    Base.errormonitor(@async try
        ...
        # `_btToolSlot` (bonitoagents.js) also finds slots on nodes the virtual
