...
struct FloatingWindow
    ...
    # decide the semantics — e.g. BonitoAgents treats close as "minimize back
...
function Bonito.jsrender(session::Bonito.Session, w::FloatingWindow)
    title_bar = DOM.div(
        DOM.span(w.title; class = "bn-fw-title-text"),
        DOM.div(DOM.span("×"; class = "bn-fw-btn", title = "Close");
                class = "bn-fw-controls");
        class = "bn-fw-title")

    container = DOM.div(
        title_bar,
        DOM.div(w.body; class = "bn-fw-body"),
        DOM.div(""; class = "bn-fw-resize", title = "Drag to resize");
    ...
    # `$(container)` resolves to a querySelector on the container's
    # `data-jscall-id`, so the script can find the element without relying on
    # `document.currentScript` (which is null when Bonito evals JSCode via
    # `new Function`).
    # Math.round on notify: pointer values are floats under subpixel layout,
    # and the Julia-side observables are `Observable{Int}` — without rounding,
    # every drag/resize end throws InexactError on `setproperty!`.
    setup = js"""(() => {
        const el = $(container);
    ...
    return Bonito.jsrender(session, DOM.div(FloatingWindowStyles, container, setup))
end
