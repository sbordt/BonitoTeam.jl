...
try
    ...
    for n in (100, 500, 1500, 3000)
        ...
        BonitoAgents.chat_emit(model, Dict{String,Any}(
    ...
    push!(model.msgs_store, BonitoAgents.AgentMsg("stream-x", ""))
    BonitoAgents.chat_emit(model, Dict{String,Any}("type" => "agent", "id" => "stream-x",
        "streaming" => true, "text" => "", "n" => length(model.msgs_store)))
    sleep(0.3)
    for i in 1:30
        BonitoAgents.chat_emit(model, Dict{String,Any}("type" => "chunk", "id" => "stream-x",
