...
try
    ...
    TH.section("Sidebar entry → swaps to the chat view") do
        ...
        BonitoAgents.set_project_title!(state, "p-1", "Dashboard nav test")
        ...
        @assert TH.wait_for(ctx, "document.body.innerText.indexOf('Workers') !== -1 || document.body.innerText.indexOf('BonitoAgents') !== -1"; timeout = 3.0)
    end
