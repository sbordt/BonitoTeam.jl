...
model = BonitoAgents.ChatModel(state, proj.server_path; project_id = proj.id,
...
lock(model.lock) do
    push!(model.msgs_store, BonitoAgents.UserMsg("please start a resource monitor"))
    push!(model.msgs_store, BonitoAgents.AgentMsg("a1", "here is your monitor"))
    push!(model.msgs_store, BonitoAgents.UserMsg("show me the lissajous plot"))
    push!(model.msgs_store, BonitoAgents.BonitoAppMsg("app1", "bonito_app", "Dashboard",
        "completed", "", time(), time(), "", "", model))
    push!(model.msgs_store, BonitoAgents.UserMsg("another unrelated question"))
end
...
try
    ...
    TH.section("save + chip + delete (global, persisted)") do
        ...
        record("persisted to the lenses file",
               @TH.test_true (sleep(0.3); isfile(ENV["BONITOAGENTS_LENSES_PATH"])))
