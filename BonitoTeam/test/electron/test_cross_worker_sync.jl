...
# ("BonitoTeam"). Because `compute_server_path` prefixes the worker name,
...
using HTTP, JSON, BonitoTeam, BonitoTeam.RemoteSync
...
function handle_xsync_inspect(mw::XSyncWorker, cmd::AbstractDict)
    ...
    resp = try
        Dict("type" => "inspect_path_response", "request_id" => rid,
             "path" => path, "summary" => BonitoTeam.BonitoWorker.inspect_path_summary(path))
    catch e
        Dict("type" => "inspect_path_response", "request_id" => rid,
...
try
    ...
    sp_a = BonitoTeam.compute_server_path(server_state, "worker-a", nm)
    sp_b = BonitoTeam.compute_server_path(server_state, "worker-b", nm)
    p_a = BonitoTeam.ProjectInfo(string(uuid4())[1:8], nm, "worker-a", sp_a, dir_a, now(UTC))
    p_b = BonitoTeam.ProjectInfo(string(uuid4())[1:8], nm, "worker-b", sp_b, dir_b, now(UTC))
    server_state.projects[][p_a.id] = p_a
    server_state.projects[][p_b.id] = p_b
    BonitoTeam.safe_notify!(server_state.projects)
    BonitoTeam.save_projects!(server_state)
    ...
    TH.section("same_name_siblings pairs across workers only") do
        sibs = BonitoTeam.same_name_siblings(server_state, p_a.id)
        ...
        record("unknown project → no siblings",
               @TH.test_true isempty(BonitoTeam.same_name_siblings(server_state, "nope")))
    end

    TH.section("compare_projects summarises both live workers") do
        cmp = BonitoTeam.compare_projects(server_state, p_a, p_b)
    ...
    TH.section("sync_across_workers! A→B is a directional overwrite") do
        BonitoTeam.sync_across_workers!(server_state, p_a, p_b)
