...
p1 = BonitoTeam.ProjectInfo("p-1", NM, "w-1", sp1, "/tmp/w1/$NM", now())
p2 = BonitoTeam.ProjectInfo("p-2", NM, "w-2", sp2, "/tmp/w2/$NM", now())
...
let model = BonitoTeam.ChatModel(state, p1.server_path;
                                  project_id = p1.id,
