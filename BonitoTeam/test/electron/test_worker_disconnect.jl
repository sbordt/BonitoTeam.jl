...
# release_locks_for_worker! should clear it on disconnect.
...
# Snapshot the version observable so we can prove bump_state! fires on
# disconnect.
version_before_anything = server_state.version[]
...
try
    ...
    TH.section("Reactive version bumped at least once across the lifecycle") do
        record("state.version[] strictly increased",
               @TH.test_true (server_state.version[] > version_before_anything))
    end
