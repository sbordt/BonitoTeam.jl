...
#
# Brings up a real BonitoTeam server, spawns two mock workers (A, B) that
...
function project_files(dir)
    ...
    for (root, _, files) in walkdir(dir)
        for f in files
            ...
            startswith(relpath(full, dir), ".bonitoTeam") && continue  # legacy
...
try
    ...
    p = BonitoTeam.ProjectInfo(pid, proj_name, "worker-a", server_dir, proj_dir_a, now(UTC))
    ...
    TH.section("Move A→B captures out-of-band edits via pre-pull") do
        ...
        BonitoTeam.transfer_project!(server_state, p, "worker-b")
    ...
    TH.section("Move B→A propagates B's edits and deletions") do
        ...
        BonitoTeam.transfer_project!(server_state, p, "worker-a")
    ...
    TH.section("Move when source is offline uses server's mirror") do
        ...
        BonitoTeam.sync_dir_to_worker!(server_state, "worker-b",
