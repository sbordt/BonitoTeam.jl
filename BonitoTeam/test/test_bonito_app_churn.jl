...
@testset "bt_show_app survives open/collapse churn in a real browser (head-of-line fix)" begin
    ...
    try
        ...
        logp = tempname() * ".log"
        appE = EC.Application(; additional_electron_args = String["--enable-logging", "--log-file=$logp", "--v=0"], sandbox = false)
...
@testset "embedded app is INTERACTIVE (browser click → worker reaction → re-render)" begin
    ...
    try
        ...
        appE = EC.Application(; additional_electron_args = String[], sandbox = false)
