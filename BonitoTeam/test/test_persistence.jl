...
function roundtrip(msgs::Vector)
    mktempdir() do dir
        ...
        session = BonitoTeam.ChatSession("sid-test", dir, now(UTC), path)
        BonitoTeam.write_session_header(session)
        for m in msgs
            if m isa BonitoTeam.UserMsg
                BonitoTeam.append_user(session, m)
            elseif m isa BonitoTeam.AgentMsg
        ...
        return BonitoTeam.load_history(session)
    end
end
...
@testset "chat.md persistence round-trip" begin
    @testset "agent message keeps block structure" begin
        ...
        End-to-end works."""
        loaded = roundtrip([BonitoTeam.AgentMsg("a1", text)])
        @test length(loaded) == 1
        @test loaded[1] isa BonitoTeam.AgentMsg
    ...
    @testset "user message round-trips" begin
        loaded = roundtrip([BonitoTeam.UserMsg("run the docs build\nplease")])
        @test length(loaded) == 1
        @test loaded[1] isa BonitoTeam.UserMsg
    ...
    @testset "mixed conversation preserves order" begin
        ...
        @test loaded[1] isa BonitoTeam.UserMsg
        @test loaded[2] isa BonitoTeam.ToolMsg
        @test loaded[3] isa BonitoTeam.AgentMsg
