...
#      conditional finalize at the BonitoTeam edge silently skipped append +
#      finished_at stamping. The bubble pulsed forever; the chat.md never
#      recorded the tool.
#
#   2. BonitoTeam `Base.close(::ToolMsg)` is TOTAL: non-terminal status at
...
# ── 2. BonitoTeam Base.close(::ToolMsg) — total ────────────────────────────
